...
def note_variants(cladestr):
    if "Delta" in cladestr:
        return 'Delta'
    elif "Beta" in cladestr:
        return 'Beta'
    elif "Alpha" in cladestr:
        return 'Alpha'
    elif "Gamma" in cladestr:
        return 'Gamma'
    elif "Eta" in cladestr:
        return 'Eta'
    elif "Iota" in cladestr:
        return 'Iota'
    elif "Kappa" in cladestr:
        return 'Kappa'
    else:
        return "None"
...
def make_plots():
    ...
    cdf["Variant"] = cdf.clade.apply(note_variants)
    bins = [1,10,50,100,500,1000,10000]
    default_frame = {k:[] for k in ["Count","Bin","Variant"]}
    for variant in cdf.Variant.value_counts().index:
        vcounts,xb = np.histogram(cdf[cdf.Variant == variant].sample_count,bins=bins)
        for b,vc in zip(xb[:-1],vcounts):
            default_frame["Count"].append(vc)
            default_frame["Bin"].append(b)
            default_frame["Variant"].append(variant)
    # counts,xb = np.histogram(cdf.sample_count,bins=bins)
    # ax = sns.barplot(x=xb[:-1],y=np.log10(counts),color='blue')
    default_frame = pd.DataFrame(default_frame)
    ax = default_frame.pivot(index="Bin",columns="Variant",values="Count").plot.bar(stacked=True)
    ax.set_xlabel("Cluster Sizes")
    ax.set_xticklabels([str(b) + "+" for b in xb[:-1]])
    # ax.set_ylabel("Count")
    #yticks = [1,2,3,4]
    #ax.set_yticks(yticks)
    #ax.set_yticklabels([str(int(10**int(y.text))) for y in ax.get_yticks()])
